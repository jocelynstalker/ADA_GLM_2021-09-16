...
!(https://lh3.googleusercontent.com/proxy/ouI7_sAQsbV8sWGH2oiPtbTykRtjQe5YKFh-00p-9VEXLc-QBTF2f4jAd-Rnp0YT8Zq4B_vbQQpdzJ3LCmuJsYXkavAkZIOsWLpoDrhF4lEiXMUisnPyNDVf) !(https://calphotos.berkeley.edu/imgs/512x768/0000_0000/0105/0567.jpeg)
...
data_newt <- select(data_newt, site, species, sex, svl, bd_poz, ttx_cm2, p_count)
```

Let's make a plot to help visualize the impact of toxicity on the number of parasites/diseases found on each newt.

```{r Figure 1. Number of parasites/diseases as a function of toxicity}
...
Running a quasipoisson changed the p-value from highly significant to insignificant. Let's turn these estimates into something that is easier to interpret. How many parasites and diseases would we expect a newt with 10,000 tetrodotoxin per cm2 to have? What about 30,000, or 50,000?.
...
```{r Calculating the Confidence Intervals of the Coefficient}
coef(newt.glm.qp)
confint(newt.glm.qp)
```
...
```{r Figure 2. Presence of Chytrid Fungus as a function of Toxicity}
